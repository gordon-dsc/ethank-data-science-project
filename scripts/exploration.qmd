...
```{r}
#save the newer data file with more relevant headlines
write_csv(over_1000, "../data/cleaned_data/full_data_2.0.csv")
```
...
```{r}
lm_model <- lm(pe_ratio ~ daily_sent + change_percent, full_data_2.0)
summary(lm_model)
```
...
```{r}
stargazer(full_data_2.0, type = "text", median = TRUE, digits = 2, title = "SPY daily price, pe ratio, and journal sentiment", out = "../data/cleaned_data/table1.txt")
```
