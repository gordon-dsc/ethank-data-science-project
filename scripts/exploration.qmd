...
Check to see if only using columns with more than 100 journals published has an affect

```{r}
column_table |>
  filter(r_square > 0) |>
  filter(num_articles >= 1000) -> over_100

wsj_data_sent |>
  filter(column %in% (over_100$journal)) |>
...
test model of all columns with at least 100 entries

```{r}
lm(pe_ratio ~ daily_sent, over_100) |>
  summary()
```
