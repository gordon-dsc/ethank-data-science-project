...
The result of running the model for through the BIC function shows the most optimaly length to lag for daily_sent is 1, or one day. This is the same as the previous lm_4.2 but can now select lag length of 1 with more confidence.
...
Optimal lag length is 4 from this run on BIC however the actual differnce in BIC value from the optimal 4 length and much simpler 1 length is small (0.0038) so I will run a model at lag length 4 and lag length 1 to compare accuracy and see if added complexity is worth it.
...
Is de-trended PE easier to predict on weekly or monthly basis?
